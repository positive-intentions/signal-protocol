...
pub(crate) fn encrypt_message_internal(
    shared_secret: &[u8],
    plaintext: &[u8],
    message_number: u32,
) -> Result<EncryptionResult, SignalError> {
...
/// This is the core logic that can be tested without WASM bindings
pub(crate) fn decrypt_message_internal(
    ciphertext: &[u8],
    message_key: &[u8],
    ...
    let nonce_ga = GenericArray::from_slice(&ciphertext[..12]);
...
pub fn encrypt_message(
    ...
    match encrypt_message_internal(&shared_secret_bytes, &plaintext_bytes, message_number) {
        Ok(result) => {
            log(&format!(
                "Message encrypted successfully, total size: {} bytes",
...
pub fn decrypt_message(
    ...
    match decrypt_message_internal(&ciphertext_bytes, &message_key_bytes) {
        Ok(plaintext) => {
...
mod tests {
    ...
    fn test_message_encryption_roundtrip() {
        ...
        let encryption_result =
            encrypt_message(&shared_secret, &plaintext, message_number).unwrap();
        ...
        // Decrypt the message
        let decrypted =
            decrypt_message(&shared_secret, &ciphertext, &message_key, message_number).unwrap();
    ...
    fn test_forward_secrecy() {
        ...
        assert_ne!(
            result1.message_key().to_vec(),
            result2.message_key().to_vec()
        );
    ...
    fn test_wrong_key_decryption_fails() {
        ...
        let decrypt_result =
            decrypt_message(&shared_secret, &ciphertext, &wrong_key, message_number);
