...
mod wasm_tests {
    ...
    use crate::rust::{
        ...
        utils::{hkdf_derive_key, serialize_public_key},
    };
...
mod native_tests {
    use super::super::{
        crypto::{
            sign_data_internal, validate_x25519_public_key, verify_signature_internal, x25519_ecdh,
        },
        double_ratchet::{
        ...
        messages::{decrypt_message_internal, encrypt_message_internal},
        types::*,
        utils::{
            deserialize_public_key_internal, hkdf_derive_key_internal,
            serialize_public_key_internal,
        },
    ...
    fn test_generate_identity_keypair_native() {
        ...
        assert_ne!(
            keypair.private_key, keypair2.private_key,
            "Generated keys should be unique"
        );
    }
    ...
    fn test_verify_signature_internal() {
        ...
        // Verify with correct public key
        let is_valid =
            verify_signature_internal(verifying_key.as_bytes(), &signature, data).unwrap();
        assert!(is_valid, "Valid signature must verify");

        // Verify with wrong data should fail
        let wrong_data = b"Different message";
        let is_invalid =
            verify_signature_internal(verifying_key.as_bytes(), &signature, wrong_data).unwrap();
        assert!(!is_invalid, "Signature with wrong data must fail");
    ...
    fn test_decrypt_message_internal() {
        ...
        let encryption_result =
            encrypt_message_internal(&shared_secret, plaintext, message_number).unwrap();

        // Decrypt the message
        let decrypted = decrypt_message_internal(
            &encryption_result.ciphertext,
            &encryption_result.message_key,
        )
        .unwrap();
    ...
    fn test_forward_secrecy_different_message_numbers() {
        ...
        let decrypted1 =
            decrypt_message_internal(&result1.ciphertext, &result1.message_key).unwrap();
        let decrypted2 =
            decrypt_message_internal(&result2.ciphertext, &result2.message_key).unwrap();
    ...
    fn test_double_ratchet_multiple_messages() {
        ...
        assert_eq!(
            double_ratchet_decrypt_internal(&mut bob_state, &enc1).unwrap(),
            msg1
        );
        assert_eq!(
            double_ratchet_decrypt_internal(&mut bob_state, &enc2).unwrap(),
            msg2
        );
        assert_eq!(
            double_ratchet_decrypt_internal(&mut bob_state, &enc3).unwrap(),
            msg3
        );
    ...
    fn test_double_ratchet_bidirectional() {
        ...
        // Alice -> Bob again
        let msg3 =
            double_ratchet_encrypt_internal(&mut alice_state, b"Alice to Bob again").unwrap();
    ...
    fn test_out_of_order_messages() {
        ...
        assert_eq!(
            double_ratchet_decrypt_internal(&mut bob_state, &msg3).unwrap(),
            b"Message 3"
        );
        assert_eq!(
            double_ratchet_decrypt_internal(&mut bob_state, &msg1).unwrap(),
            b"Message 1"
        );
        assert_eq!(
            double_ratchet_decrypt_internal(&mut bob_state, &msg2).unwrap(),
            b"Message 2"
        );
    ...
    fn test_cleanup_skipped_message_keys_none_to_remove() {
        ...
        state
            ...
            .insert("key1".to_string(), vec![1u8; 32]);
        state
            .skipped_message_keys
            .insert("key2".to_string(), vec![2u8; 32]);
